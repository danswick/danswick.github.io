...
function joinTileset(map, tileset_id, foreign_key, data, type, stops) {
    // joinTilesets from - https://github.com/mapbox/labs/blob/mb-pages/dataset-join/join.js
    // for this demo, there are only 2 stops: on and off

    var categoricalStops = [["0", "rgba(0,0,0,0)"]]; // The first value in the array is used as a default with opacity=0
    var keys = Object.keys(data);
    keys.forEach(function(k) {
        if (isNaN(data[k])) return;
        var value = "rgba(0,0,0," + isVisited() + ")";
        categoricalStops.push([k, value]);
    });

    map.setPaintProperty(tileset_id, "fill-color", {property: foreign_key, type: "categorical", stops: categoricalStops});

    // Hide other tilesets
    for (var i = 0; i < join.tilesets.length; i++) {
        map.setLayoutProperty(join.tilesets[i].name, "visibility", join.tilesets[i].name !== tileset_id ? "none" : "visible");
    }
}

function isVisited(feature) {
    var visitedVal;
    if (feature.properties.visited === 'no') {
        visitedVal  = 1;
    } else if (feature.properties.visited === 'yes') {
        visitedVal = 0;
    }
